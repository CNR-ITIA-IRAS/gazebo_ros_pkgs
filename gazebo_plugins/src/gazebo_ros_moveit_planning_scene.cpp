...
void GazeboRosMoveItPlanningScene::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  ROS_INFO("NICOLA ---- Robot Namespace: '%s'", this->robot_namespace_.c_str() );
  ...
  ROS_INFO("NICOLA ---- Robot name: '%s'", this->robot_name_.c_str() );
  ...
  ROS_INFO("NICOLA ---- Topic Name: '%s'", this->topic_name_.c_str() );
  ...
  ROS_INFO("NICOLA ---- Scene Name: '%s'", this->scene_name_.c_str() );
  ...
  ROS_INFO("NICOLA ---- Update period: %f", this->publish_period_.toSec() );

  std::string static_elements = "NA";
  if (!_sdf->HasElement("staticElements")) 
  {
    this->static_elements_.clear();
  } 
  else 
  {
    static_elements = _sdf->GetElement("staticElements")->Get<std::string>();
    boost::algorithm::split(this->static_elements_, static_elements, boost::algorithm::is_any_of(" \n"));
  }
  ROS_INFO("NICOLA ---- static_elements (n. elements: %zu):", this->static_elements_.size() );
  for( const auto & s : this->static_elements_ )
    ROS_INFO(" - %s", s.c_str() );
...
//     if( std::find( this->static_elements_.begin(), this->static_elements_.end(), object.id ) == this->static_elements_.end() )
//       continue;
//     
